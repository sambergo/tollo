...
pub fn initialize_database() -> Result<Connection> {
    ...
    if list_count == 0 {
        conn.execute(
            ...
            &["Default", "https://iptv-org.github.io/iptv/index.m3u", "1"],
        )?;
    }
...
pub fn disable_all_groups(conn: &mut Connection, channel_list_id: i64, groups: Vec<String>) -> RusqliteResult<()> {
...
mod tests {
    ...
    fn test_populate_channels_success() {
        ...
        // Verify channel data
        let (name, url): (String, String) = conn.query_row(
            "SELECT name, url FROM channels WHERE id = 1",
            [],
    ...
    fn test_sync_channel_list_groups() {
        ...
        let all_groups: Vec<String> = conn.prepare("SELECT group_name FROM group_selections WHERE channel_list_id = 1")
            .unwrap()
    ...
    fn test_enable_all_groups() {
        ...
        let groups = vec!["Sports".to_string(), "News".to_string(), "Movies".to_string()];
    ...
    fn test_save_and_get_saved_filters() {
        ...
        // Save some filters
        save_filter(&conn, 1, 0, "test search".to_string(), Some("Sports".to_string()), "My Filter".to_string()).unwrap();
        save_filter(&conn, 1, 1, "another search".to_string(), None, "Another Filter".to_string()).unwrap();
    ...
    fn test_saved_filter_replace() {
        ...
        // Save a filter
        save_filter(&conn, 1, 0, "original search".to_string(), Some("Sports".to_string()), "Original".to_string()).unwrap();
        
        // Replace it with a new filter
        save_filter(&conn, 1, 0, "new search".to_string(), Some("News".to_string()), "New Filter".to_string()).unwrap();
    ...
    fn test_cascade_delete() {
        ...
        conn.execute("DELETE FROM channel_lists WHERE id = 1", []).unwrap();
        
        // Verify dependent data was deleted
        let group_count: i64 = conn.query_row("SELECT COUNT(*) FROM group_selections WHERE channel_list_id = 1", [], |row| row.get(0)).unwrap();
        assert_eq!(group_count, 0);
        
        let filter_count: i64 = conn.query_row("SELECT COUNT(*) FROM saved_filters WHERE channel_list_id = 1", [], |row| row.get(0)).unwrap();
    ...
    mod error_tests {
        ...
        fn test_save_filter_with_invalid_slot_numbers() {
            ...
            assert!(result.is_err(), "Negative slot should fail CHECK constraint");
        ...
        fn test_unicode_and_emoji_handling() {
            ...
            for (i, group_name) in unicode_cases.iter().enumerate() {
                ...
                assert!(result.is_ok(), "Failed to handle Unicode group: {}", group_name);
                ...
                assert!(result.is_ok(), "Failed to save Unicode filter: {}", group_name);
            }
