...
export class AsyncPlaylistStore {
  ...
  async addPlaylistAsync(name: string, source: string): Promise<number> {
    try {
      const id = await invoke<number>('validate_and_add_channel_list_async', { name, source });
  ...
  async getFetchStatus(id: number): Promise<PlaylistFetchStatus | null> {
    try {
      return await invoke<PlaylistFetchStatus | null>('get_playlist_fetch_status', { id });
    } catch (error) {
  ...
  async getAllFetchStatuses(): Promise<PlaylistFetchStatus[]> {
    try {
      return await invoke<PlaylistFetchStatus[]>('get_all_playlist_fetch_status');
    } catch (error) {
  ...
  isPlaylistFetching(id: number): boolean {
    ...
    return status?.status === 'fetching' || status?.status === 'processing' || status?.status === 'saving';
  }
